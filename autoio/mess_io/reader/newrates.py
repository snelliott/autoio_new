...
def read(output_str, reactant, product):
    ...
    # Read the high-pressure and pressure-dependent rate constants
    high_kt_dct = _highp_ks(mess_lines, reaction)



def _highp_kts(mess_lines, reaction):
    """ Parses the MESS output file string for the rate constants [k(T)]s
        for a single reaction at the high-pressure limit.
...
def _pdep_kts(output_str, reactant, product, pressure):
...
def _parse_rate_constants(mess_lines, block_start, reaction, pressure):
    ...
    for i in range(block_start+1, len(mess_lines)):
        if mess_lines[i].strip() == '':
            break
        temps.append(mess_lines[i].strip().split()[reaction_col])
        kts.append(mess_lines[i].strip().split()[reaction_col])
