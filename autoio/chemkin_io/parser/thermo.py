...
def get_temp_limits(entry, default_midpoint):
    ...
    try:
        ...
        if default_midpoint is None:
            raise ImportError(
                f'No default mipoint temp in the file and no midpoint temp for the following entry:\n{formatted_entry}'
            )
        else:
            midpoint = default_midpoint
            print(
                f'Using default midpoint temperature, {default_midpoint}, for the following entry:\n{formatted_entry}'
        )
