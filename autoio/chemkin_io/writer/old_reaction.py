...
def lindemann(reaction, high_params, low_params, colliders=(), ea_units='kcal/mol'):
    """ Write the string containing the Lindemann fitting parameters
        formatted for ChemKin input files
...
def _format_rxn_name(rxn_key):
    """ Receives a rxn key from a param_dct and
        writes it to a string that the above functions
        can handle.
    """
    rcts = rxn_key[0]
    prds = rxn_key[1]
